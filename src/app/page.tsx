...
export default function Home() {
  ...
  const streakValidated = useRef(false);
  const setLanguage = useStore((s) => s.setLanguage);
  const appMode = useStore((s) => s.settings.appMode) ?? "fun";
  ...
  useEffect(() => {
    ...
    // Streak validation: run ONCE only (ref guard prevents re-runs)
    if (hasPersisted && !streakValidated.current) {
      streakValidated.current = true;
      try {
        ...
        if (raw) {
          ...
          if (parsed.state && parsed.state.lastFartDay) {
            ...
            const twoDaysAgoKey = fmt(new Date(Date.now() - 2 * 86400000));
